...
def check_in():
    text1 = "This is the text passed from variable"
    window2 = Tk()
    window2.geometry("700x500")
    print("enter user details")
    label = tk.Label(window2,text=text1,padx=0,pady=20).grid(column=500, row=0)

    tk.Button(window2, text="Quit" ,command= window2.destroy).grid(column=900, row=100)
    
    
def check_out():
    text2 = "Welcome come to Check OUT"
    window3 = Tk()
    window3.geometry("700x500")
    print("enter user details")
    label = tk.Label(window3,text= text2,padx=0,pady=20).grid(column=5000, row=0)
